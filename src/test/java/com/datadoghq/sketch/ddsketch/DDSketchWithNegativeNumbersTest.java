...
abstract class DDSketchWithNegativeNumbersTest extends QuantileSketchTest<DDSketchWithNegativeNumbers> {
    ...
    protected void assertAccurate(boolean merged, double[] sortedValues, double quantile, double actualQuantileValue) {
        ...
        if (actualQuantileValue >= 0) {
            lowerQuantileValue = sortedValues[(int) Math.floor(quantile * (sortedValues.length - 1))];
            upperQuantileValue = sortedValues[(int) Math.ceil(quantile * (sortedValues.length - 1))];
            minExpected = lowerQuantileValue * (1 - relativeAccuracy());
            maxExpected = upperQuantileValue * (1 + relativeAccuracy());
        } else {
            lowerQuantileValue = sortedValues[(int) Math.ceil(quantile * (sortedValues.length - 1))];
            upperQuantileValue = sortedValues[(int) Math.floor(quantile * (sortedValues.length - 1))];
            minExpected = upperQuantileValue * (1 + relativeAccuracy());
            maxExpected = lowerQuantileValue * (1 - relativeAccuracy());
        }
